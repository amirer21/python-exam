"""데이터 시각화"""
#파이썬의 Pandas 라이브러리를 이용해 데이터를 분석하고 시각화하는 방법을 알아보자.
#데이터 시각화는 데이터를 이해하는데 도움이 되며, 데이터 분석 결과를 다른 사람에게 효과적으로 전달할 수 있다.


# 순서 1) 지도 데이터 읽어오기(서울시 행정 경계 지도 데이터)
# 순서 2) 지도 데이터 불러오기(서울시 행정 경계 지도 데이터)
# 순서 3) 기본 지도 만들기
# - 서울시 시군구 경계 지도 그리기
